...
class Learner:
    ...
    def __init__(self,
        ...
        pre_fc1 = tf.concat(1, [conv2_flat, old_ffn_])
        ...
        def function_get_best_action(state, old_ffn):
            fetches = [best_a, fc1]
            feed_dict={s1_: state, old_ffn_: old_ffn}
            best_action, new_fnn = self.session.run(fetches=fetches, feed_dict=feed_dict)
            return best_action, new_fnn
    ...
    def learn_from_memory(self):
        ...
        if self.memory.size > self.batch_size:
            ...
            if not self.sequential_updates:
                ...
                sequences = self.memory.get_sequence(self.batch_size)

    def perform_learning_step(self, game, actions, epoch, reward_exploration, old_ffn):
        ...
        a, new_ffn = self.fn_get_best_action(s1, old_ffn)
    ...
    def learn(self, game, actions, visual=False, reward_exploration=False):
        ...
        for epoch in range(self.epochs):
            ...
            self.positions = []
            score = 0
            old_ffn = np.zeros([1, 128])
            for learning_step in trange(self.learning_steps_per_epoch):
                self.perform_learning_step(game, actions, epoch, reward_exploration, old_ffn)
                if reward_exploration:
